...
class ClangIndexer():
    ...
    def __run_on_directory(self, id, args):
        ...
        if not self.symbol_db_exists():
            ...
            # Create a dummy empty input list file for argparse compatibility
            dummy_input_handle, dummy_input_filename = create_indexer_input_list_file(self.root_directory, '.cxxd_interactive_dummy', [])
            os.close(dummy_input_handle)
            indexer_input_list.append(dummy_input_filename) # Keep track to delete later? Actually existing code appends to cleaning list
            ...
            for i in range(num_cores):
                ...
                workers.append(proc)
                indexing_subprocess_list.append(proc)
            # Scheduler State
            import select
            import time
            ...
            logging.info("Dynamic Indexing Completed.")
...
def index_file_list(root_directory, input_filename_list, compiler_args_filename, output_db_filename, worker_id=None):
    ...
    logging.info(f"{worker_prefix}Indexing process started: {total_files} files, {total_bytes / (1024*1024):.2f} MB total.")
    ...
    for filename, size in file_sizes:
        ...
        # Log progress every ~5% or every 100 files
        pct = (processed_bytes / total_bytes * 100) if total_bytes > 0 else 100.0
        if pct - last_logged_pct >= 5 or processed_files % 100 == 0 or processed_files == total_files:
            logging.info(f"{worker_prefix}Progress: {processed_files}/{total_files} files ({pct:.1f}% size) completed")
            last_logged_pct = pct
    symbol_db.close()
def index_interactive(root_directory, compiler_args_filename, output_db_filename, worker_id=None):
    import sys
    symbol_db = SymbolDatabase(output_db_filename)
    symbol_db.create_data_model()
    cxxd_config_parser = CxxdConfigParser(os.path.join(root_directory, '.cxxd_config.json'), root_directory)
    # Use global cache for TUs in this process
    parser = ClangParser(compiler_args_filename, TranslationUnitCache(NoCache()), cxxd_config_parser.get_clang_library_file())
...
def indexer_visitor(ast_node, ast_parent_node, args):
    ...
    parser, symbol_db, root_directory, symbol_buffer = args
...
def index_single_file(parser, root_directory, filename, symbol_db):
    ...
    if tunit:
        symbol_buffer = []
        parser.traverse(tunit.cursor, [parser, symbol_db, root_directory, symbol_buffer], indexer_visitor)
        if symbol_buffer:
            symbol_db.insert_symbol_entries_batch(symbol_buffer)
...
def get_cpp_file_list(root_directory, blacklisted_directories, recognized_file_extensions):
    ...
    for dirpath, dirs, files in os.walk(root_directory):
        # Optimization: prune blacklisted directories from traversal
...
def start_indexing_subprocess(root_directory, compiler_args_filename, indexer_input_list_filename, output_db_filename, log_filename, worker_id, interactive=False):
    cmd_args = [
    ...
    cmd = " ".join([f"'{arg}'" if ' ' in arg or '=' in arg or arg == '' else arg for arg in cmd_args])
    # Legacy quoted string construction for non-interactive (shell style) if needed, 
    # but strictly subprocess.Popen(["arg", ...]) is safer than shell=True string.
    # The original code used shlex.split(quoted_string), so it was effectively using list args.
    # We can just use list args directly for both.
