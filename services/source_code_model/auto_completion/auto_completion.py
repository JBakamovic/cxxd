...
class AutoCompletion():
    ...
    def __code_complete(self, id, args):
        ...
        current_char_idx = column - 1
        if current_char_idx < len(line_string):
            current_char = line_string[current_char_idx]
            logging.info("current_char = '{0}'".format(current_char))
            if current_char == '\n' or current_char == '\r':
                logging.info('Carriage return detected ...')
                del self.completion_candidates[:]
            elif current_char.isspace() or current_char == ',' or current_char == ';' or current_char == '=':
                logging.info('Delimiting character detected ... Clearing up the completion candidates.')
                del self.completion_candidates[:]
            else:
                current_char_idx += 1
                logging.info("Will extract symbol from '{0}'".format(line_string[0:current_char_idx]))
                logging.info('Trying harder ...')
                idx = extract_symbol(line_string[0:current_char_idx])
                if idx != -1:
                    expression = line_string[(current_char_idx-idx-1):current_char_idx]
                    logging.info("expression found = '{0}'".format(expression))
                    current_char = expression[len(expression)-1] if len(expression) else ''
                    logging.info("current_char = '{0}'".format(current_char))
                    delimiter = (self.previous_char == ':' and current_char == ':') or (self.previous_char == '-' and current_char == '>') or current_char == '.'
                    if len(self.completion_candidates) == 0 or delimiter:
                        ...
                        self.completion_candidates = self.__filter_completion_candidates(
                            ...
                            expression.strip() if not delimiter else ''
                        ) # At this point we might already have something to work with (e.g. part of the string we may trigger filtering with)
                    else:
                        logging.info(
                            "Filtering by expression = '{0}' from '{1}' starting from {2}".format(expression, line_string[0:current_char_idx], idx)
                        )
                        self.completion_candidates = self.__filter_completion_candidates(
                            self.completion_candidates,
                            expression.strip() if not delimiter else ''
