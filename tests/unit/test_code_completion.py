...
from services.code_completion.code_completion import CodeCompletion
from services.code_completion.code_completion import CodeCompletionRequestId
from services.code_completion.code_completion import CodeCompletionSortingAlgorithmId
...
class CodeCompletionTest(unittest.TestCase):
    ...
    def setUp(self):
        self.service = CodeCompletion(self.parser)
...
int main() {                                \n\
    return 0;                               \n\
        ...
        success, completion_candidates = self.service([
            CodeCompletionRequestId.CODE_COMPLETE,
            self.fd.name, self.fd.name,
            line,
            column,
            self.line_to_byte(45, line),
            CodeCompletionSortingAlgorithmId.BY_PRIORITY
        ])
...
int main() {                                \n\
    return ate                              \n\
        ...
        success, completion_candidates = self.service([
            ...
            column,
            self.line_to_byte(45, line),
            CodeCompletionSortingAlgorithmId.BY_ALPHABET
        ])
        self.assertEqual(success, True)
...
int main() {                                \n\
    ...
    def test_if_call_returns_true_and_non_empty_candidate_list_when_code_completing_after_parenthesis(self):
        self.fd.write('\
struct Config { static const int N = 10; }; \n\
int main()                                  \n\
{C                                          \n\
...
int main() {                                \n\
    ...
    def test_if_call_returns_true_and_non_empty_candidate_list_is_returned_when_trying_to_codecomplete_local_function(self):
        self.fd.write('\
struct P { int x; int y; };                 \n\
P create_p(int x, int y) {return {x, y};}   \n\
int main() {                                \n\
...
int main() {                                \n\
    ...
    def test_if_call_returns_true_and_code_completion_is_triggered_only_once_and_further_refinements_are_done_by_filtering_existing_candidates(self):
        self.fd.write('\
struct P { int x; int y; };                 \n\
P create_p1(int x, int y) {return {x, y};}  \n\
P create_p2(int x, int y) {return {x, y};}  \n\
int main() {                                \n\
    return create_p                         \n\
        ...
        with mock.patch.object(self.service, '_CodeCompletion__filter_completion_candidates', return_value=candidates.results) as mock_filter_completion_candidates:
            line, column = 5, 19
            success, completion_candidates = self.service([
                CodeCompletionRequestId.CODE_COMPLETE,
                self.fd.name, self.fd.name,
...
int main() {                                \n\
    return create_p1                        \n\
        ...
        with mock.patch.object(self.service, '_CodeCompletion__filter_completion_candidates', return_value=candidates.results[0:1]) as mock_filter_completion_candidates:
            line, column = 5, 20
            success, completion_candidates = self.service([
                CodeCompletionRequestId.CODE_COMPLETE,
                self.fd.name, self.fd.name,
...
int main() {                                \n\
    return create                           \n\
        ...
        success, completion_candidates = self.service([
            ...
            column,
            self.line_to_byte(45, line),
            CodeCompletionSortingAlgorithmId.BY_KIND
        ])
        self.assertEqual(success, True)
        self.assertEqual(len(completion_candidates), 5)
    ...
    def test_if_call_returns_true_and_code_completion_doesnt_work_when_inside_the_udt(self):
        self.fd.write('\
int my_var = 59;                            \n\
class Point {                               \n\
public:                                     \n\
...
private:                                    \n\
  int _x; int _y; int _z;                   \n\
    ...
    def test_if_call_returns_true_and_macro_code_completion_doesnt_work(self):
        self.fd.write('\
#define MY_VAR 5                            \n\
int main() {                                \n\
MY                                          \n\
...
int main() {                                \n\
    return 0;                               \n\
        ...
        line, column = 5, 1
        success, completion_candidates = self.service([
            CodeCompletionRequestId.CACHE_WARMUP,
            self.fd.name,
            line,
            column
        ])
